...
func (l *Lexer) ReadNumber() (*Token, error) {
	...
	for {
		...
		if r == '.' {
			if readDeci {
				return nil, fmt.Errorf("two decimal places in number")
			}
			if read == "" {
				read = "0"
			}
	...
	if read == "" {
		return nil, fmt.Errorf("no number was read")
	}

	if readDeci {
		return NewToken(tDOUBLE, read), nil
	}
	return NewToken(tNUMBER, read), nil
}
